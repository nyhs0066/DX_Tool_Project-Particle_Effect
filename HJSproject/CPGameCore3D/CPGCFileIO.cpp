...
BYTE*               CPGCFileIOManager::m_pfileBuf = 0;
LARGE_INTEGER		CPGCFileIOManager::m_fileSize;
DWORD				CPGCFileIOManager::m_numberOfFragment = 0;
DWORD				CPGCFileIOManager::m_curOPCount = 0;
HANDLE				CPGCFileIOManager::m_hOPStart = 0;
HANDLE				CPGCFileIOManager::m_hProgressPrint = 0;

CPGCFileIOManager::CPGCFileIOManager()
{
...
bool CPGCFileIOManager::release()
{
    return true;
}

size_t CPGCFileIOManager::loadFile(std::wstring wszFilePath, void* destLoadData)
{
    bool bRet = true;
...
size_t CPGCFileIOManager::saveFile(std::wstring wszFilePath, void* srcSaveData, UINT iBytes)
{
    init();
...
UINT CPGCFileIOManager::asyncReadProc(LPVOID argList)
{
    //파일 이름 캐스팅
...
UINT CPGCFileIOManager::asyncWriteProc(LPVOID argList)
{
    //파일 이름 캐스팅
    const WCHAR* wFilename = (WCHAR*)argList;
...
bool CPGCFileIOManager::asyncReadSub(HANDLE handle, DWORD dwOffset, LARGE_INTEGER& readBytes, OVERLAPPED& readOV)
{
    DWORD dwRead = 0;
    bool bPending = false;
...
bool CPGCFileIOManager::asyncWriteSub(HANDLE handle, DWORD dwOffset, LARGE_INTEGER& writeBytes, OVERLAPPED& writeOV)
{
    DWORD dwWrite = 0;
    bool bPending = false;
...
bool getSplitName(std::string& szFullPath, std::wstring& wszOutfileName)
{
    ...
    //파일 경로 분해
    _wsplitpath_s(api_M2W(szFullPath.c_str()), drive, dir, fileName, extension);
