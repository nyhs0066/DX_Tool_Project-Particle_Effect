//////////////////////////////////////
//
// PS_PNCT_NoTexWithLight.hlsl
//		PNCT정점 구조를 사용하는 Pixel shader
//		라이트를 반영하는 정점 컬러를 그대로 반환한다.
// 
//////////////////////////////////////
		...
		for (int u = -iHalf; u <= iHalf; u++)
		{
			...
			fLightIntensity +=
				g_shadowMap0.SampleCmpLevelZero(g_smapCompSampler,
					float3(shadowUnit.xy + vOffset, 0),
...
PS_OUTPUT PS(PS_INPUT pIn)
{
	...
	//최종 색상
	float4 FinalColor = pIn.color * (AmbientLight + DiffuseLight + SpecularLight) * ShadowAttenuation;
	FinalColor.a = 1.0f;

	float3 camDir = pIn.worldVPos.xyz - camPos.xyz;
	normalize(camDir);
