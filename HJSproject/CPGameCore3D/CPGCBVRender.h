...
public:
	...
	void setVertices(DirectX::BoundingOrientedBox& OBB)
	{
		...
		CPM_Matrix rot = CPM_Matrix::CreateFromQuaternion(OBB.Orientation);

		Axis[0] = CPM_Vector3::Transform(Axis[0], rot);
		Axis[1] = CPM_Vector3::Transform(Axis[1], rot);
		Axis[2] = CPM_Vector3::Transform(Axis[2], rot);
	...
	void setVertices(DirectX::BoundingSphere& Sphere)
	{
		...
		CPM_Vector3 unit = { 1.0f * Sphere.Radius, 0.0f, 0.0f };
		...
		DirectX::BoundingOrientedBox& OBB)
	{
		...
		m_shaders.pVS = CPGC_SHADER_MGR.getVS(L"VS_LineObject");
		m_shaders.pPS = CPGC_SHADER_MGR.getPS(L"PS_LineObject");

		if (m_pDevice)
		{
			int size = m_vertices.size() * sizeof(LineVertex);
			createDXSimpleBuf(m_pDevice,
				size,
				&m_vertices.at(0),
				D3D11_BIND_VERTEX_BUFFER,
				m_pVBuf.GetAddressOf());

			size = sizeof(CPRS_CBUF_COORDCONV_MATSET);
			createDXSimpleBuf(m_pDevice,
				size,
				&m_wvpMat,
	...
	void update(CPM_Matrix* pWorld, CPM_Matrix* pView, CPM_Matrix* pProj)
	{
		...
		if (m_pDContext)
		{
			...
			if (SUCCEEDED(hr))
			{
				memcpy(ms.pData, &m_wvpMat, sizeof(CPRS_CBUF_COORDCONV_MATSET));
	...
	void render()
	{
		if (m_pDContext)
		{
			...
			m_pDContext->IASetInputLayout(CPGC_VLAYOUT_MGR.getPtr(L"VL_PC"));
